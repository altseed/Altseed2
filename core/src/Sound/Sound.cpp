...
std::shared_ptr<Sound> Sound::Load(const char16_t* path, bool isDecompressed) {
    ...
    if (!soundMixer->m_isSoundMixerEnabled) {
        LOG_CRITICAL(u"Sound::Load: failed, sound is disabled");
        return nullptr;
    }
    ...
    if (sound == nullptr) {
        Log::GetInstance()->Error(
                LogCategory::Core, u"Sound::Load: Failed to create sound from '{0}'", utf16_to_utf8(path).c_str());
        return nullptr;
    }
