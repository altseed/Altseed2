...
bool Joystick::IsPresent(int32_t joystickIndex) { return (bool)handler[joystickIndex]; }
...
ButtonState Joystick::GetButtonStateByIndex(int32_t joystickIndex, int32_t buttonIndex) const {
    ...
    if (currentHit[joystickIndex][buttonIndex] && preHit[joystickIndex][buttonIndex])
        return ButtonState::Hold;
    else if (!currentHit[joystickIndex][buttonIndex] && preHit[joystickIndex][buttonIndex])
...
JoystickType Joystick::GetJoystickType(int32_t index) const { return types[index]; };

char16_t* Joystick::GetJoystickName(int32_t index) const { return (char16_t*)names[index]; }
