...
namespace altseed{
    ...
    void Mouse::SetWheelCallback(std::function<void(double x, double y)> func)
    {
        wheelCallback = func;
    }
    ...
    ButtonState Mouse::GetMouseButtonState(MouseButtons button) const
    {
        ...
        if (currentState[index] && oldState[index]) return ButtonState::Hold;
        else if (!currentState[index] && oldState[index]) return ButtonState::Release;
        else if (currentState[index] && !oldState[index]) return ButtonState::Push;
