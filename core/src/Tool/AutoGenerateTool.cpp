...
void Tool::PushID(const char16_t* str_id_begin, const char16_t* str_id_end) {
    RETURN_IF_NULL(str_id_begin, );
    RETURN_IF_NULL(str_id_end, );
...
bool Tool::Selectable(const char16_t* label, bool* p_selected, ToolSelectableFlags flags, Vector2F size) {
    RETURN_IF_NULL(label, false);
    RETURN_IF_NULL(p_selected, false);
...
void Tool::CalcListClipping(int32_t items_count, float items_height, int32_t* out_items_display_start, int32_t* out_items_display_end) {
    RETURN_IF_NULL(out_items_display_start, );
    RETURN_IF_NULL(out_items_display_end, );
...
void Tool::LoadIniSettingsFromMemory(const char16_t* ini_data, int32_t ini_size) {
    RETURN_IF_NULL(ini_data, );
    ImGui::LoadIniSettingsFromMemory(utf16_to_utf8(ini_data).c_str(), ini_size);
}
...
const char16_t* Tool::SaveIniSettingsToMemory(int32_t* out_ini_size) {
    returnStr = utf8_to_utf16(ImGui::SaveIniSettingsToMemory(out_ini_size));
    return returnStr.c_str();
}

bool Tool::DebugCheckVersionAndDataLayout(const char16_t* version_str, int32_t sz_io, int32_t sz_style, int32_t sz_vec2, int32_t sz_vec4, int32_t sz_drawvert, int32_t sz_drawidx) {
    RETURN_IF_NULL(version_str, false);
    return ImGui::DebugCheckVersionAndDataLayout(utf16_to_utf8(version_str).c_str(), sz_io, sz_style, sz_vec2, sz_vec4, sz_drawvert, sz_drawidx);
}
