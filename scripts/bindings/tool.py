...
with ToolCond as enum_:
    enum_.brief = cbg.Description()
    enum_.brief.add('en', 'Do NOT combine multiple values using binary operators')
    ...
    with enum_.add('Once', 1 << 1) as v:
        v.brief= cbg.Description()
        v.brief.add('en', 'Set the variable once per runtime session (only the first call with succeed)')
        v.brief.add('ja', 'ランタイムセッションごとに変数を1回設定します（成功した最初の呼び出しのみ）')
    
    with enum_.add('FirstUseEver', 1 << 2) as v:
        v.brief= cbg.Description()
        v.brief.add('en', 'Set the variable if the object/window has no persistently saved data (no entry in .ini file)')
        v.brief.add('ja', 'オブジェクト/ウィンドウに永続的に保存されたデータがない場合（.iniファイルにエントリがない場合）、変数を設定します')
    
    with enum_.add('Appearing', 1 << 3) as v:
        v.brief= cbg.Description()
        v.brief.add('en', 'Set the variable if the object/window is appearing after being hidden/inactive (or the first time)')
        v.brief.add('ja', 'オブジェクト/ウィンドウが非表示/非アクティブになった後（または初めて）表示される場合は、変数を設定します')
...
with ToolTreeNode as enum_:
    ...
    with enum_.add('AllowItemOverlap', 1 << 2) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Hit testing to allow subsequent widgets to overlap this one')
    
    with enum_.add('NoTreePushOnOpen', 1 << 3) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Don\'t do a TreePush() when open (e.g. for CollapsingHeader) = no extra indent nor pushing on ID stack')
    
    with enum_.add('NoAutoOpenOnLog', 1 << 4) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Don\'t automatically and temporarily open node when Logging is active (by default logging will automatically open tree nodes)')
    ...
    with enum_.add('Leaf', 1 << 8) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'No collapsing, no arrow (use as a convenience for leaf nodes).')
    ...
    with enum_.add('SpanFullWidth', 1 << 12) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Extend hit box to the left-most and right-most edges (bypass the indented area).')
    
    with enum_.add('NavLeftJumpsBackHere', 1 << 13) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '(WIP) Nav: left direction may move to this TreeNode() from any of its child (items submitted between TreeNode and TreePop)')
...
with ToolInputText as enum_:
    ...
    with enum_.add('EnterReturnsTrue', 1 << 5) as v:
        ...
        v.brief.add('ja', '（値が変更されるたびにではなく）Enterが押されたときに `true` を返します。 `IsItemDeactivatedAfterEdit()` 関数を調べることを検討してください。')
    ...
    with enum_.add('CallbackHistory', 1 << 7) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Callback on pressing Up/Down arrows (for history handling)')
        v.brief.add('ja', '上下矢印を押すとコールバック（履歴処理用）')
    
    with enum_.add('CallbackAlways', 1 << 8) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Callback on each iteration. User code may query cursor position, modify text buffer.')
    ...
    with enum_.add('CallbackCharFilter', 1 << 9) as v:
        ...
        v.brief.add('ja', '置換または破棄する文字入力のコールバック。 \'EventChar\'を変更して置換または破棄するか、コールバックで1を返して破棄します。')
    
    with enum_.add('AllowTabInput', 1 << 10) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Pressing TAB input a \'\\t\' character into the text field')
    ...
    with enum_.add('CtrlEnterForNewLine', 1 << 11) as v:
        ...
        v.brief.add('ja', '複数行モードでは、Enterでフォーカスを外し、Ctrl + Enterで新しい行を追加します（デフォルトは反対です：Ctrl + Enterでフォーカスを外し、Enterで行を追加します）。')
    ...
    with enum_.add('NoUndoRedo', 1 << 16) as v:
        ...
        v.brief.add('ja', '元に戻す/やり直しを無効にします。 アクティブな間は入力テキストがテキストデータを所有していることに注意してください。独自の元に戻す/やり直しスタックを提供する場合は、たとえば ClearActiveID（）を呼び出します。')
...
with ToolColorEdit as enum_:
    ...
    with enum_.add('NoAlpha', 1 << 1) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker, ColorButton`: ignore Alpha component (will only read 3 components from the input pointer).')
        v.brief.add('ja', '`ColorEdit, ColorPicker, ColorButton`: Alphaコンポーネントを無視します（入力ポインターから3つのコンポーネントのみを読み取ります）。')

    with enum_.add('NoPicker', 1 << 2) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit`: disable picker when clicking on colored square.')
        v.brief.add('ja', '`ColorEdit`: 色付きの正方形をクリックしたときにピッカーを無効にします。')

    with enum_.add('NoOptions', 1 << 3) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit`: disable toggling options menu when right-clicking on inputs/small preview.')
        v.brief.add('ja', '`ColorEdit`: 入力/小さなプレビューを右クリックしたときのオプションメニューの切り替えを無効にします。')

    with enum_.add('NoSmallPreview', 1 << 4) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker`: disable colored square preview next to the inputs. (e.g. to show only the inputs)')
        v.brief.add('ja', '`ColorEdit, ColorPicker`: 入力の横にある色付きの正方形プレビューを無効にします。 （例：入力のみを表示する）')

    with enum_.add('NoInputs', 1 << 5) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker`: disable inputs sliders/text widgets (e.g. to show only the small preview colored square).')
        v.brief.add('ja', '`ColorEdit, ColorPicker: 入力スライダー/テキストウィジェットを無効にします（たとえば、小さなプレビューの色付きの四角形のみを表示します）。')

    with enum_.add('NoTooltip', 1 << 6) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker, ColorButton`: disable tooltip when hovering the preview.')
        v.brief.add('ja', '`ColorEdit, ColorPicker, ColorButton`: プレビューをホバーするときにツールチップを無効にします。')

    with enum_.add('NoLabel', 1 << 7) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'ColorEdit, ColorPicker: disable display of inline text label (the label is still forwarded to the tooltip and picker).')
        v.brief.add('ja', '`ColorEdit, ColorPicker`: インラインテキストラベルの表示を無効にします（ラベルは引き続きツールチップとピッカーに転送されます）。')

    with enum_.add('NoSidePreview', 1 << 8) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorPicker` : disable bigger color preview on right side of the picker, use small colored square preview instead.')
        v.brief.add('ja', '`ColorPicker`: ピッカーの右側の大きなカラープレビューを無効にし、代わりに小さな色付きの正方形プレビューを使用します。')

    with enum_.add('NoDragDrop', 1 << 9) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit`: disable drag and drop target. `ColorButton`: disable drag and drop source.')
        v.brief.add('ja', '`ColorEdit`: ドラッグアンドドロップターゲットを無効にします。 `ColorButton`: ドラッグアンドドロップソースを無効にします。')

    with enum_.add('AlphaBar', 1 << 16) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker`: show vertical alpha bar/gradient in picker.')
        v.brief.add('ja', '`ColorEdit, ColorPicker`: ピッカーに垂直アルファバー/グラデーションを表示します。')

    with enum_.add('AlphaPreview', 1 << 17) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker, ColorButton`: display preview as a transparent color over a checkerboard, instead of opaque.')
        v.brief.add('ja', '`ColorEdit, ColorPicker, ColorButton`: プレビューを不透明ではなく、チェッカーボード上の透明色として表示します。')

    with enum_.add('AlphaPreviewHalf', 1 << 18) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker, ColorButton`: display half opaque / half checkerboard, instead of opaque.')
        v.brief.add('ja', '`ColorEdit, ColorPicker, ColorButton`: 不透明ではなく、半不透明/半市松模様を表示します。')
    ...
    with enum_.add('HDR', 1 << 19) as v:
        ...
        v.brief.add('ja', '`(WIP) ColorEdit`: 現在、RGBAエディションで0.0f..1.0fの制限のみを無効にします（注：おそらくFloatフラグも使用したいでしょう）。')

    with enum_.add('DisplayRGB', 1 << 20) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit`: override _display_ type among RGB/HSV/Hex. ColorPicker: select any combination using one or more of RGB/HSV/Hex.')
        v.brief.add('ja', '`ColorEdit`: RGB/HSV/Hexの_display_タイプをオーバーライドします。 `ColorPicker`: 1つ以上のRGB/HSV/Hexを使用して任意の組み合わせを選択します。')
    ...
    with enum_.add('Uint8', 1 << 23) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker, ColorButton`: _display_ values formatted as 0..255.')
        v.brief.add('ja', '`ColorEdit, ColorPicker, ColorButton`: 0..255としてフォーマットされた_display_値。')
    ...
    with enum_.add('Float', 1 << 24) as v:
        ...
        v.brief.add('ja', '`ColorEdit, ColorPicker, ColorButton`: _display_値は、0..255整数ではなく0.0f..1.0f浮動小数点としてフォーマットされます。 整数による値の往復はありません。')
    ...
    with enum_.add('PickerHueWheel', 1 << 26) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorPicker`: wheel for Hue, triangle for Sat/Value.')
        v.brief.add('ja', '`ColorPicker`: Hueのホイール、Sat/Valueの三角形。')

    with enum_.add('InputRGB', 1 << 27) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '`ColorEdit, ColorPicker`: input and output data in RGB format.')
        v.brief.add('ja', '`ColorEdit, ColorPicker`: RGB形式の入出力データ')

    with enum_.add('InputHSV', 1 << 28) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'ColorEdit, ColorPicker: input and output data in HSV format.')
...
with ToolSelectable as enum_:
    ...
    with enum_.add('SpanAllColumns', 1 << 1) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Selectable frame can span all columns (text will still fit in current column)')
    ...
    with enum_.add('AllowItemOverlap', 1 << 4) as v:
        v.brief = cbg.Description()
        v.brief.add('en', '(WIP) Hit testing to allow subsequent widgets to overlap this one')
...
with ToolWindow as enum_:
    ...
    with enum_.add('NoScrollbar', 1 << 3) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable scrollbars (window can still scroll with mouse or programmatically)')
    ...
    with enum_.add('NoScrollWithMouse', 1 << 4) as v:
        ...
        v.brief.add('ja', 'ユーザーがマウスホイールで垂直にスクロールできないようにします。 子ウィンドウでは、NoScrollbarも設定されていない限り、マウスホイールは親に転送されます。')

    _NoCollapse = 1 << 5
    with enum_.add('NoCollapse', _NoCollapse) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable user collapsing window by double-clicking on it')
    ...
    with enum_.add('NoBackground', 1 << 7) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable drawing background color (WindowBg, etc.) and outside border. Similar as using SetNextWindowBgAlpha(0.0f).')
        v.brief.add('ja', '描画背景色(`WindowBg`など)および外枠を無効にします。 `SetNextWindowBgAlpha(0.0f)`を使用する場合と同様です。')
    ...
    with enum_.add('NoMouseInputs', 1 << 9) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable catching mouse, hovering test with pass through.')
    ...
    with enum_.add('HorizontalScrollbar', 1 << 11) as v:
        ...
        v.brief.add('ja', '水平スクロールバーの表示を許可します（デフォルトではオフ）。 `Begin()`を呼び出す前に、`SetNextWindowContentSize(Vector2F(width, 0.0f));`を使用して幅を指定できます。')
    
    with enum_.add('NoFocusOnAppearing', 1 << 12) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable taking focus when transitioning from hidden to visible state')
        v.brief.add('ja', '非表示から表示状態に移行するときにフォーカスを取得できないようにします')
    
    with enum_.add('NoBringToFrontOnFocus', 1 << 13) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable bringing window to front when taking focus (e.g. clicking on it or programmatically giving it focus)')
        v.brief.add('ja', 'フォーカスを取得するときにウィンドウを前面に移動することを無効にします（たとえば、クリックするか、プログラムでフォーカスを与える）')
    
    with enum_.add('AlwaysVerticalScrollbar', 1 << 14) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Always show vertical scrollbar (even if `ContentSize.Y < Size.Y`)')
        v.brief.add('ja', '常に垂直スクロールバーを表示します（`ContentSize.Y < Size.Y`の場合でも）')
    
    with enum_.add('AlwaysHorizontalScrollbar', 1 << 15) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Always show horizontal scrollbar (even if `ContentSize.x < Size.x`)')
    ...
    with enum_.add('AlwaysUseWindowPadding', 1 << 16) as v:
        ...
        v.brief.add('ja', '境界線のない子ウィンドウが`style.WindowPadding`を使用するようにします（境界線のない子ウィンドウではデフォルトで無視されるため、より便利です）')
    ...
    with enum_.add('NoNavFocus', _NoNavFocu) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'No focusing toward this window with gamepad/keyboard navigation (e.g. skipped by CTRL+TAB)')
        v.brief.add('ja', 'ゲームパッド/キーボードナビゲーションでこのウィンドウにフォーカスしない（たとえば、CTRL + TABでスキップ）')
    ...
    enum_.add('NoDecoration', _NoTitleBar | _NoResize | _NoScrollbar | _NoCollapse)
...
with ToolTabBar as enum_:
    ...
    with enum_.add('Reorderable', 1 << 0) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Allow manually dragging tabs to re-order them + New tabs are appended at the end of list')
    ...
    with enum_.add('NoTabListScrollingButtons', 1 << 4) as v:
        v.brief = cbg.Description()
        v.brief.add('en', 'Disable scrolling buttons (apply when fitting policy is FittingPolicyScroll)')
        v.brief.add('ja', 'スクロールボタンを無効にする（フィッティングポリシーが`FittingPolicyScroll`の場合に適用）')
...
with Tool as class_:
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     # func_.add_arg('isOpen', *bool) # TODO
    #     func_.return_value.type_ = bool
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     func_.add_arg(float*, 'value') # TODO
    #     func_.return_value.type_ = bool
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     func_.add_arg(float*, 'array') # TODO
    #     func_.return_value.type_ = bool
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     func_.add_arg(float*, 'angle') # TODO
    #     func_.return_value.type_ = bool
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     func_.add_arg(int*, 'currentMin') # TODO
    #     func_.add_arg(int*, 'currentMax') # TODO
    #     func_.add_arg(float, 'speed')
    #     func_.add_arg(int, 'valueMin')
    #     func_.add_arg(int, 'valueMax')
    #     func_.return_value.type_ = bool
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     func_.add_arg(float*, 'currentMin') # TODO
    #     func_.add_arg(float*, 'currentMax') # TODO
    #     func_.add_arg(float, 'speed')
    #     func_.add_arg(float, 'valueMin')
    #     func_.add_arg(float, 'valueMax')
    #     func_.return_value.type_ = bool
    ...
    #     func_.brief.add('ja', '')
    #     func_.add_arg(ctypes.c_wchar_p, 'label')
    #     func_.add_arg(Vector4F*, 'color') # TODO
